...
let switcher_rate = true;
let increase = true;

function sortByRate(arr) {
    switcher_rate ? switcher_rate = false : switcher_rate = true;
    if (switcher_rate)
        return arr = arr.sort((a, b) => collator.compare(a.ratingRevievs, b.ratingRevievs));

    else
        return arr = arr.sort((a, b) => collator.compare(b.ratingRevievs, a.ratingRevievs));
...
function sortByPrice(arr) {
    ...
    arr.sort((a, b) => {
        ...
        let firstPrice = +(a.price.newUan ? a.price.newUan : a.price).replace(/\D/g, "");
        let secondPrice = +(b.price.newUan ? b.price.newUan : b.price).replace(/\D/g, "");
...
const sort_rate_btn = document.querySelector(".sort-options .sort-rate");
...
function displayList(array = []) {
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        rating.classList.add('item-title');
        rating.innerText = item.ratingRevievs;
        item_element.appendChild(rating);

        let price = document.createElement('div');
        price.classList.add('item-title');
        price.innerText = (item.price.newUan ? item.price.newUan : item.price);
...
sort_rate_btn.addEventListener('click', () => {
    let array = sortByRate(rate);
    displayList(array);
});

sort_price_btn.addEventListener('click', () => {
    increase ? increase = false : increase = true;

    let array = sortByPrice(rate);
    displayList(array);
});